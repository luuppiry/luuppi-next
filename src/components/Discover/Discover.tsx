		...
		<section className="max-w-screen-xl mx-auto py-20 px-4">
			...
			<div className="grid grid-cols-3 max-lg:grid-cols-2 gap-6">
				{links.map((link) => (
					<Link
						href={link.href}
						className="group relative w-full h-full flex aspect-[3/2] border-4 border-black shadow-[7px_7px_#000] hover:shadow-[0px_0px_#000] transition-all duration-300 max-md:aspect-square"
						key={link.name}
					>
						<div className="absolute w-0 h-full bg-[#7b7fb5]/50 z-20 group-hover:w-full transition-all duration-300"></div>
						...
						<div className="relative flex w-full h-full overflow-hidden">
							<div className="w-full h-full flex absolute z-10 bg-gradient-to-t via-transparent from-[#7b7fb5]/70 to-transparent"></div>
							<Image
								src={link.image}
								layout="fill"
								objectFit="cover"
